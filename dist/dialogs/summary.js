...
var _require = require('humanize-plus');

const oxford = _require.oxford;

const pick = require('lodash/fp/pick');
const pluck = require('lodash/fp/pluck');

const fields = [{ name: 'type', display: 'App or bot?' }, { name: 'newOrExisting', display: 'New or existing?' }, { name: 'platforms', display: 'Platforms' }, { name: 'users', display: '# of users' }, { name: 'description', display: 'Description' }, { name: 'stack', display: 'Tech stack' }, { name: 'history', display: 'History' }, { name: 'devs', display: '# of devs' }, { name: 'team', display: 'Work with team?' }, { name: 'location', display: 'Location' }, { name: 'startDate', display: 'Start date' }, { name: 'contact', display: 'Contact' }];
...
const giveSummary = session => {
  ...
  session.send(data.newOrExisting === 'greenfield' ? `You want to build a new ${data.type}.` : `You need help with an existing ${data.type}`);

  session.send(data.platforms.find(equals('Not sure')) ? 'You\'re not sure what platforms you\'re interested in.' : `You're interested in ${oxford(data.platforms)} platforms.`);

  session.send(data.users === 'Not sure' ? 'You\'re not sure how many users you expect' : `You anticipate ${data.users} users.`);
  ...
  session.send(data.team === 'not sure' ? 'You\'re not sure whether you want us to work with a team or alone.' : `Make Apps will be working ${data.team}.`);

  session.send(`You are located in ${data.location}`);

  session.send(data.startDate === 'not sure' ? 'You\'re not sure when you want to get started.' : `You want to get started ${data.startDate}`);
...
module.exports = [giveSummary, handleConfirmation];
